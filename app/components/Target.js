...
class TargetPage extends Component {
  ...
  render() {
    const {dispatch, treecounter} = this.props;
    return (
      <div>
        <div className="text-center">{this.state.loading ? <LoadingIndicator/> : null}</div>
        <LiForm schema={this.state.schema} initialValues={this.state.initialValues}
                onSubmit={(treecounterData) => this.props.SubmitTarget(treecounterData, treecounter.id)}
                baseForm={CustomForm}
                headline=""
                buttonText={this.state.label} buttonWidth="240"/>
      </div>
    );
  }
