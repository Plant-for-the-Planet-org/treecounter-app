...
class Menu extends Component {
	...
	fetchAndSetMenuState() {
		Sidebar.MenuAction(this.props.loggedIn).then(({ data }) => {
	...
	sideNavImage() {
		return <div className="app-container__sidenav--image">
	...
	render() {
		...
		return (
			<div className={"app-container__sidenav " + (this.props.isOpen ? "open" : "")}>
				{this.sideNavImage()}
				{this.state.data.map(element => <div key={"div" + element.sequence}>
						<span className="app-container__sidenav--heading">{element.caption}</span>
						<ul className="app-container__sidenav--list" key={element.sequence}>
							{element.menu_items.map(menuItem => (
								menuItem.enabled ?
