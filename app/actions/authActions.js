  ...
  return dispatch => {
    request
      .then(res => {
        ...
        const payload = {token, user: {...jwtDecode(token), ...res.data.data}};
        dispatch(setUserLogIn(payload));

        NotificationManager.success("Login Successful", "Welcome", 5000);
        history.push({pathname: getLocalRoute("app_userHome"), state: {id: res.data.data.id}}); // TODO: understand what this is doing
      ...
      .catch((error) => {
        if (error !== undefined && error.response !== undefined && error.response.status === 401) {
          ...
          NotificationManager.error(error.response.data.message, error.response.data.code, 5000);
        } else {
...
export function forgot_password(data) {
  axios.post(getApiRoute('auth_forgotPassword_post'), data)
    .then(res => {
      debug(res.status);
      NotificationManager.success("Further details have been sent to your mail address");
...
export function reset_password(data) {
  ...
  axios.post(getApiRoute('auth_resetPassword_post'), data)
    .then(res => {
      debug(res.status);
    }).catch(err => debug(err));
}
