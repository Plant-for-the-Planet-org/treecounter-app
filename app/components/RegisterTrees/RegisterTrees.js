    ...
    return (
      <div>
        <div className="row flex-row f-center">
          <h3 className="cs-heading">{constants.formStrings.registerPlantedTrees}</h3>
        </div>
        <div className="register-tree">
          <form className="register-tree__type">
            <label
              className={"radio register-tree__type--option " + (this.state.treeCount === 'individual' ? 'active' : '')}>
              <input
                type="radio"
                value="individual"
            ...
            <label className={"radio register-tree__type--option " + (this.state.treeCount === 'many' ? 'active' : '')}>
              <input
                type="radio"
                value="many"
        ...
        <div className={"register-tree__form " + (this.state.treeCount === 'individual' ? 'hide-treecount' : '' )}>
          {this.state.loading ? <div className="center-wrapper">
              <LoadingIndicator/>
            </div> :
            <LiForm schema={this.state.schema} onSubmit={(e) => this.handleRegisterTreeSubmit(e)} baseForm={CustomForm}
                    headline="" buttonText="Register" buttonWidth="240"/>
          }
        </div>
      </div>
    );
