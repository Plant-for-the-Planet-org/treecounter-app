...
export const getApiRoute = (routeName, params) => {
  ...
  debug("route context:", `serverName: ${serverName}, baseUrl: ${baseUrl}, locale: ${locale}, serverRendered: ${serverRendered}`);

  const host = serverRendered ? '' : serverName;

  const url = `${host}${baseUrl}${Routing.generate(routeName, {_locale: locale, ...params})}`;
...
export const getLocalRoute = (routeName, params) => {
  ...
  debug("route context:", `baseUrl: ${baseUrl}, locale: ${locale}, serverRendered: ${serverRendered}`);

  //const base = serverRendered ? baseUrl : '';

  const url = `${baseUrl}${Routing.generate(routeName, {_locale: locale, ...params})}`;
  debug(`generated route from '${routeName}': ${url}`);
  return url;
};
