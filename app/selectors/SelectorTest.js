...
const SelectorTest = ({state}) => {
  debug('SELECTOR currentUserProfileSelector', currentUserProfileSelector(state));
  ...
  debug('SELECTOR sortedUserContributionsSelector', sortedUserContributionsSelector(state));
  debug('SELECTOR selectedPlantProjectSelector', selectedPlantProjectSelector(state));
  debug('SELECTOR availablePaymentGatewaysSelector', availablePaymentGatewaysSelector(state));
  debug('SELECTOR userTreecounterDataSelector', userTreecounterDataSelector(state));
  debug('SELECTOR userPlantProjectsSelector', userPlantProjectsSelector(state));
  debug('SELECTOR sortedUserPlantProjectsSelector', sortedUserPlantProjectsSelector(state));
  debug('SELECTOR activePlantProjectsSelector', activePlantProjectsSelector(state));
  debug('SELECTOR unusedPlantProjectsSelector', unusedPlantProjectsSelector(state));
  return <span></span>
};
